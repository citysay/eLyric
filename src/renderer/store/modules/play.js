...
const actions = {
  ...
  playMoveTime({ state, commit }, sec) {
    commit(types.PLAY_MOVE_TIME, sec)
  }
}
...
const mutations = {
  ...
  [types.PLAY_MOVE_TIME](state, sec) {
    if (sec === 0) {
      return
    }
    const tTime = state.currentTime + sec
    if (sec > 0) {
      state.tmpCurrentTime = Math.min(state.durationTime, tTime)
      return
    }
    state.tmpCurrentTime = Math.max(0, tTime)
    console.log(sec, state.tmpCurrentTime);
  },
  ...
  [types.SET_MUSIC_LIST](state, songs) {
    ...
    items.forEach(item => {
      ...
      // 只对添加到播放列表起作用
      if (item.addUrl) {
        api.loadUrl({ id: item.id }, (data) => { item.location = data.data[0].url })
      }
      if (!flag) {
